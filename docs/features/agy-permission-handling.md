...
- `packages/happy-cli/src/agy/runStreamJsonAgy.ts`：会话入口，初始化 `AgyPermissionHandler` 并挂载至 `session.rpcHandlerManager`，监听 `tool-call` / `tool-result` 事件并同步状态至 `agentState.requests`。
