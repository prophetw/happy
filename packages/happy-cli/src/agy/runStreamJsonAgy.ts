...
export interface RunStreamJsonAgyOptions {
  ...
  /** Raw --engine value from the CLI; interpreted by the runAgy dispatcher. */
  engine?: string;
  /** Pins the backend factory: 'cli' forces the stream-json engine, 'sdk' the Python bridge. */
  forceEngine?: 'sdk' | 'cli';
}

export async function runStreamJsonAgy(opts: RunStreamJsonAgyOptions): Promise<void> {
